...
class CheckersCommand extends UserCommand
{
    protected $name = 'checkers';

    private $query_prefix = 'g3';
    private $strings = [
        'game_empty_field' => ' ',
    ];

    private $rules_url = 'https://en.wikipedia.org/wiki/Straight_checkers';
                        ...
                        if ($data['current_selection'] != '') {
                            if ($data['current_selection'] == $command[0].$command[1]) {
                                ...
                                if (in_array($command[0].$command[1], $possibleMoves['valid_moves']) && $data['board'][$command[0]][$command[1]] == '') {
                                    ...
                                    if ($killed && $canChain && $this->possibleMoves($data['board'], $command[0].$command[1], true)) {
