...
class TictactoeCommand extends UserCommand
{
    protected $name = 'tictactoe';

    private $query_prefix = 'g1';
    private $strings = [
        'game_empty_field' => ' ',
    ];

    private $rules_url = 'https://en.wikipedia.org/wiki/Tic-tac-toe';
                ...
                for ($y = 0; $y < $this->max_y; $y++) {
                    if ($board[$x][$y] == 'X' || $board[$x][$y] == 'O' || strpos($board[$x][$y], 'win')) {
                        if ($winner == 'X' && $board[$x][$y] == 'O') {
                            $field = $this->strings['O_lost'];
                        } elseif ($winner == 'O' && $board[$x][$y] == 'X') {
                            $field = $this->strings['X_lost'];
                        } elseif (isset($this->strings[$board[$x][$y]])) {
